...
1. Алгоритмы контролируемого обучения обучаются с использованием маркированных данных, что означает, что входные данные помечены правильным выходом. Цель этих алгоритмов — изучить сопоставление входных данных с выходными данными, что позволяет предсказывать выходные данные для новых данных. Обычные алгоритмы контролируемого обучения включают:
    ...
    * Дерево решений: Алгоритм дерева решений в машинном обучении является одним из самых популярных алгоритмов, используемых сегодня; это алгоритм контролируемого обучения, который используется для классификации проблем. Он хорошо работает при классификации как категориальных, так и непрерывных зависимых переменных. Этот алгоритм делит популяцию на два или более однородных набора на основе наиболее значимых атрибутов/независимых переменных.
    ...
    * Q-learning: это алгоритм обучения с подкреплением, не использующий модель, который изучает значение действия в определенном состоянии.
    https://habr.com/ru/companies/otus/articles/803041/
    * Глубокие Q-сети (DQN): сочетают в себе Q-обучение и глубокие нейронные сети, что позволяет изучать успешные стратегии непосредственно на основе многомерных сенсорных данных.
    https://habr.com/ru/companies/wunderfund/articles/671650/
    * Методы градиента политики: эти методы напрямую оптимизируют параметры политики, а не оценивают ценность действий.
    https://habr.com/ru/articles/555202/
    * Поиск по дереву Монте-Карло (MCTS): используется в процессах принятия решений для нахождения оптимальных решений путем проигрывания сценариев, в частности, в таких играх, как го.
    ...
    * Алгоритм KNN (K-ближайших соседей).Этот алгоритм можно применять как к задачам классификации, так и к задачам регрессии. По-видимому, в отрасли Data Science он более широко используется для решения задач классификации. Это простой алгоритм, который хранит все доступные случаи и классифицирует любые новые случаи, принимая большинство голосов своих k соседей. Затем случай назначается классу, с которым у него больше всего общего. Функция расстояния выполняет это измерение.
        ...
        Что следует учесть перед выбором алгоритма K ближайших соседей: 
    ...
    * Алгоритмы снижения размерности. В современном мире корпорации, государственные учреждения и исследовательские организации хранят и анализируют огромные объемы данных. Как специалист по данным, вы знаете, что эти необработанные данные содержат много информации — задача состоит в том, чтобы выявить значимые закономерности и переменные.
    Алгоритмы снижения размерности , такие как дерево решений, факторный анализ, отношение пропущенных значений и случайный лес, могут помочь вам найти релевантные данные.

        https://habr.com/ru/articles/751050/

        https://habr.com/ru/companies/newprolab/articles/350584/
    * Алгоритм усиления градиента и алгоритм AdaBoosting. Алгоритмы Gradient Boosting и AdaBoosting — это алгоритмы повышения, которые используются, когда необходимо обрабатывать большие объемы данных для прогнозирования с высокой точностью. Повышение — это алгоритм обучения ансамбля, который объединяет предсказательную силу нескольких базовых оценщиков для повышения надежности.
    Короче говоря, он объединяет несколько слабых или средних предикторов для создания сильного предиктора. Эти алгоритмы повышения всегда хорошо работают в соревнованиях по науке о данных, таких как Kaggle, AV Hackathon, CrowdAnalytix. Это самые предпочтительные алгоритмы машинного обучения на сегодняшний день. Используйте их вместе с Python и R Codes для достижения точных результатов.
